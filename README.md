...
6. Multi-answer questions still use two different type strings in different parts of the app: the admin UI and bulk upload use `MULTIPLE_CHOICE_MULTIPLE`, while the backend creation logic and student render logic also look for `MULTIPLE_CHOICE_MULTI`. That mismatch can store a question as multi-answer in one place but render or score it as single-answer in another. See [front/app/admin/questions/QuestionForm.tsx](front/app/admin/questions/QuestionForm.tsx#L27), [gobackend/handlers_questions.go](gobackend/handlers_questions.go#L266), and [front/app/student/exam/[id]/page.tsx](front/app/student/exam/[id]/page.tsx#L995).

7. The current test flow now has the major ownership and flush gaps reduced, but the remaining server-side start gate is still the main correctness blocker. If you want one next fix, that should be the `handleStartTest` deadline logic, because it directly controls whether students can actually begin the exam on time.
